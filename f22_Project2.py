...
.
    ...
    #finding policy number, adding it to information_list
        ...
        elif pattern1.search(policy_number) or pattern2.search(policy_number)  or pattern3.search(policy_number):
...
def check_policy_numbers(data):
    ...
    for tup in data:
        #print (tup[3])
        if pattern1.search(tup[3]) or pattern2.search(tup[3]):
           count = 1
        elif tup[3] == 'Pending':
           count = 1
        elif tup[3] == 'Exempt':
...
def extra_credit(listing_id):
    ...
    for date in review_dates:
        review_years.append(re.findall("([0-9]{4})", date.get_text()))
